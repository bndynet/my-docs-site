...
- Seeds `blog/welcome.md` + `blog/authors.yml` if there's no `blog/` dir yet.
  The scaffolded `docs.config.js` turns the blog plugin on (default) and adds
  a `Blog` nav entry — to disable it, set `blog: false` in `docs.config.js`,
  remove the nav entry, and delete the `blog/` directory.
...
module.exports = {
  ...
  mdxRoot: './docs',
  blog: true,
  // Each entry in `nav` is one top menu item. `docsPath` builds an
  // autogenerated sidebar from <mdxRoot>/<docsPath>, keyed by `label`.
  nav: [
